...
- Add ```check-dependencies``` to ```prebuild``` and ```pretest``` lifecycle. This lib checks if the developer have the up-to-date dependencies inside its node_modules directory, as described in the ```package.json``` file.
